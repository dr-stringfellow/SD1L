...
def PlotFitResults(frame,fitErrs,nPars,pulls,data_name,pdf_names,chi2,ndof,canvname, plot_dir, has_sig = False, draw_sig = False, plot_label = "", ratio_unc = None, maxy=1E6, miny=2):
    ...
    frame.SetMinimum(1E-2*miny)
    frame.SetMaximum(1E2*maxy)
    frame.SetName("mjjFit")
    frame.GetYaxis().SetTitle("Events / 20 GeV")
    print("############################")
    print(frame.GetXaxis().GetNdivisions())
    print("############################")
    frame.SetTitle("")
    frame.Draw()
        
    legend = ROOT.TLegend(0.52097293,0.4583362,0.7381766,0.679833)
    legend2 = ROOT.TLegend(0.52097293,0.4583362,0.7381766,0.679833)
    ...
    legend.AddEntry(frame.findObject(data_name),"#bf{Data}","lpe")
    if(len(pdf_names[0]) > 1):
        fit_name = pdf_names[0]
    else: fit_name = pdf_names
    if(not has_sig or not draw_sig): 
        legend.AddEntry(frame.findObject(fit_name),"#bf{%i par. background fit}"%nPars,"l")
        legend.AddEntry("","","")
        legend.AddEntry("","","")
    ...
    pt = ROOT.TPaveText(0.2,0.06,0.58,0.17,"NDC")
    pt.SetTextFont(42)
    pt.SetTextAlign(12)
    pt.SetFillColor(0)
    pt.SetBorderSize(0)
    ...
    pt2 = ROOT.TPaveText(0.2,0.76,0.6,0.86,"NDC")
    pt2.SetTextFont(42)
    pt2.SetTextSize(0.056)
    pt2.SetTextAlign(11)
    pt2.SetFillColor(0)
    pt2.SetBorderSize(0)
    pt2.SetFillStyle(0)
    ...
    pt3 = ROOT.TPaveText(0.192,0.89,0.6,0.99,"NDC")
    pt3.SetTextFont(42)
    pt3.SetTextAlign(12)
    pt3.SetFillColor(0)
    pt3.SetBorderSize(0)
    pt3.SetFillStyle(0)
    pt3.AddText("#bf{CMS}#scale[0.8]{#it{ Preliminary}}")
    pt3.Draw()


    pt4 = ROOT.TPaveText(0.752,0.89,0.94,0.99,"NDC")
    pt4.SetTextFont(42)
    pt4.SetTextAlign(12)
    pt4.SetFillColor(0)
    pt4.SetBorderSize(0)
    pt4.SetFillStyle(0)
    pt4.AddText("(13.6 TeV)")
    pt4.Draw()
    ...
    c1.cd(2)
    p11_2 = c1.GetPad(2)
    p11_2.SetPad(0.01,0.02,0.99,0.275)
    ...
    pulls.GetXaxis().SetNdivisions(510)
...
def load_h5_sb(h_file, hist, correctStats=False, sb1_edge = -1., sb2_edge = -1.,fraction=1.0):
    event_num = None
    with h5py.File(h_file, "r") as f:
        mjj = np.array(f['mjj'][()])
        mjj = mjj[:int(fraction*len(mjj))]
        if(correctStats):
